...
export default async function HomePage({ searchParams }: HomePageProps) {
  ...
  function pageHref(next: number) {
    return `/?provider=${encodeURIComponent(providerSlug)}&page=${next}`;
  }
    ...
    <div>
      ...
      <nav className="tabs" aria-label="Providers">
        {providers.map((provider) => (
          <Link
            key={provider.id}
            href={`/?provider=${encodeURIComponent(provider.slug)}&page=1`}
      ...
      {events.length === 0 ? (
        <div className="card">
          <p style={{ margin: 0 }}>No events yet for {providerSlug}.</p>
      ...
      <div className="pagination">
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          {prevPage ? (
            <Link href={pageHref(prevPage)} className="btn">
              Previous
            </Link>
          ) : (
          ...
          {nextPage ? (
            <Link href={pageHref(nextPage)} className="btn">
              Next
            </Link>
          ) : (
            <span className="btn" aria-disabled>
